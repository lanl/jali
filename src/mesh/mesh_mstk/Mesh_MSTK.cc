...
MSet_ptr Mesh_MSTK::build_set(const JaliGeometry::RegionPtr region,
  ...
  case CELL:    // cellsets      
    ...
    if (region->type() == JaliGeometry::BOX ||
      ...
      int ncell = num_entities(CELL, ALL);              
      ...
      int nnode = num_entities(NODE, ALL);
  ...
  case FACE:  // sidesets
    ...
    if (region->type() == JaliGeometry::BOX)  {
      ...
      int nface = num_entities(FACE, ALL);
  ...
  if (region->type() == JaliGeometry::LOGICAL) {
    JaliGeometry::LogicalRegionPtr boolregion = (JaliGeometry::LogicalRegionPtr) region;
