...
TEST(MESH_TILES_MPI) {
  ...
  for (int i = 0; i < numframeworks; i++) {
    ...
    int ncells_on_proc = mesh->num_cells<Jali::OWNED>();
    ...
    for (auto const & t : mesh->tiles()) {
      ...
      int nnodes_owned = t->num_nodes<Jali::OWNED>();
      int nnodes_ghost = t->num_nodes<Jali::GHOST>();
//      CHECK_EQUAL(9, nnodes_owned + nnodes_ghost);

      // check that we can iterate through the nodes correctly

      int nnodes = 0;
      for (auto n : t->nodes<Jali::OWNED>())
        nnodes++;
      for (auto n : t->nodes<Jali::GHOST>())
        nnodes++;
//    CHECK_EQUAL(9, nnodes);
      ...
      if (edges_requested) {
        ...
        int nedges_owned = t->num_edges<Jali::OWNED>();
        int nedges_ghost = t->num_edges<Jali::GHOST>();
//        CHECK_EQUAL(12, nedges_owned + nedges_ghost);

        // check that we can iterate through the edges correctly
        int nedges = 0;
        for (auto e : t->edges<Jali::OWNED>())
          nedges++;
        for (auto e : t->edges<Jali::GHOST>())
          nedges++;
 //       CHECK_EQUAL(12, nedges);
      ...
      if (faces_requested) {
        ...
        int nfaces_owned = t->num_faces<Jali::OWNED>();
        int nfaces_ghost = t->num_faces<Jali::GHOST>();
//        CHECK_EQUAL(12, nfaces_owned + nfaces_ghost);

        // check that we can iterate through the faces correctly
        int nfaces = 0;
        for (auto f : t->faces<Jali::OWNED>())
          nfaces++;
        for (auto f : t->faces<Jali::GHOST>())
          nfaces++;
//        CHECK_EQUAL(12, nfaces);
      ...
      if (wedges_requested) {
        // We should have 48 OWNED wedges per cell

        int nwedges = 0;
        for (auto w : t->wedges<Jali::OWNED>())
          nwedges++;

        CHECK_EQUAL(48*ncells_per_tile, nwedges);
        int nwedges_owned = t->num_wedges<Jali::OWNED>();
        CHECK_EQUAL(nwedges, nwedges_owned);
        int nwedges_ghost = t->num_wedges<Jali::GHOST>();
      ...
      if (corners_requested) {
        ...
        int ncorners = 0;
        for (auto cn : t->corners<Jali::OWNED>())
          ncorners++;

        CHECK_EQUAL(8*ncells_per_tile, ncorners);
        int ncorners_owned = t->num_corners<Jali::OWNED>();
        CHECK_EQUAL(ncorners, ncorners_owned);
        int ncorners_ghost = t->num_corners<Jali::GHOST>();
