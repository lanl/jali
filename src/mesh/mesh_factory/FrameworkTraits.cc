...
class bogus_mesh : public Jali::Mesh {
 public:
  ...
  bogus_mesh(const char *filename, const MPI_Comm& comm,
             ...
             const bool request_sides, const bool request_wedges,
  ...
  : Mesh(request_faces, request_edges, request_sides, request_wedges,
         ...
         num_ghost_layers_boundary, partitioner, geom_type, comm) {
  ...
  void edge_get_nodes_internal(const Jali::Entity_ID edgeid,
...
framework_generate(const MPI_Comm& comm, const Framework& f,
  ...
  default:
    {
      std::string msg =
        boost::str(boost::format("unknown mesh framework: %d")
                   % static_cast<int>(f));
      Exceptions::Jali_throw(Errors::Message(msg.c_str()));
    }
  }
