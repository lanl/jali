...
/*! @class MeshTile "MeshTile.hh"
    @brief Encapsulates notion of a meshtile (submesh) on a compute node

    Mesh tiles are small non-overlapping(*) chunks of a mesh on a
    compute node and are typically obtained by partitioning the full
    mesh on a compute node.  They have (almost?) all the functionality
    of a mesh partition in a distributed mesh. Meshtiles are a way for
    the work on a mesh to be broken up into manageable, parallely
    executable chunks.

    (*) Like meshes, meshtiles have a notion of owned entities and
    ghost entities. Owned entities reside only on this tile and their
    data can be updated by work processes on this partition. Ghost
    entities are "reflections" of entities owned other partitions and
    can only be "updated" by the owning partition, not by any work
    processes executing on this partition
...
class Meshtile
{
 ...
 public:
  ...
  Meshtile(Mesh & const parent_mesh, std::vector<int> const meshcellids,
           int nghostlayers=1)  {}
  ...
  /// Get parallel type of entity - Parallel_type::OWNED, Parallel_type::GHOST, Parall_type::Parallel_type::ALL (OWNED+GHOST)

  Parallel_type entity_get_ptype(const Entity_kind kind,
                                 const Entity_ID entid) const;


  /*! @brief mesh entity ID

    Get the mesh entity ID corresponding to this tile entity. This is
    different from the global ID across the entire distributed mesh
    (Think tile_entity-->mesh_entity-->global_entity  - this operator
    is getting the second given the first).
  */


  Entity_ID mesh_entity_ID(const Entity_kind kind,
                           const Entity_ID tile_entity_id) const;
  ...
  //! particular category (Parallel_type::OWNED, Parallel_type::GHOST, Parall_type::Parallel_type::ALL)
  ...
  node_iterator end_owned_nodes() {
    return nodeids.begin()+num_entities(Entity_kind::NODE,Parallel_type::OWNED);
  }
  ...
  edge_iterator end_owned_edges() {
    return edgeids.begin()+num_entities(Entity_kind::EDGE,Parallel_type::OWNED);
  }
  ...
  face_iterator end_owned_faces() {
    return faceids.begin()+num_entities(Entity_kind::FACE,Parallel_type::OWNED);
  }
  ...
  cell_iterator end_owned_cells() {
    return cellids.begin()+num_entities(Entity_kind::CELL,Parallel_type::OWNED);
  }
  ...
  //! cell, Parallel_type::OWNED or Parallel_type::GHOST. If ordered = true, the faces will be
  ...
  //! face, Parallel_type::OWNED or Parallel_type::GHOST. If ordered = true, the edges will be
  //! returned in a ccw order around the face as it is naturally defined.
  //!
  //! IMPORTANT NOTE IN 2D Entity_kind::CELLS: In meshes where the cells are two
